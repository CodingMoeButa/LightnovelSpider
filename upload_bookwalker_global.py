...
local_sqlite_result = local_sqlite_cur.execute('''SELECT pid,title,author,illustrator,publisher,
release_year,release_month,release_day,description
FROM bookwalker_global
WHERE submit=0''')
for local_sqlite_row in list(local_sqlite_result):
    ...
    release_day = local_sqlite_row[7]
    ...
    img_path = './img/bookwalker_global/'+str(int(int(pid)/10000))+'/'+str(pid)+'.jpg'
    ...
    local_sqlite_cur.execute('UPDATE bookwalker_global SET submit=1 WHERE pid=?', (pid,))
